...
The core issue is that **Pith's prose generation prompts ask for summaries, not specifics**. The current prompts request:
- Summary (1 sentence)
- Purpose (2-3 sentences)
- Gotchas (warnings)
- Data flow (general description)

But they don't capture:
- Specific values from code (retry counts, timeouts, thresholds)
- Line numbers for important logic
- Concrete code snippets as evidence
- Cross-module transformations with function names

### Why Efficiency Improved but Actionability Didn't

The +4 point improvement came primarily from:
1. **Better efficiency** (5.0 vs 2.8) - Prose is more concise
2. **Better accuracy** (4.6 vs 3.6) - Descriptions are more correct

But actionability remained low (2.0) because:
- Concise prose still lacks implementation specifics
- Accurate summaries don't include "where" information
- No mechanism to capture values/constants from code

### The Synthesis Problem

Even when Pith has relevant data (line numbers, code snippets added in Phase 6.6), the LLM prose generation doesn't reference it. The generated prose reads like documentation, not like a developer's notes with specific pointers.

**Example**: For retry logic, Pith could theoretically know `maxRetries=3` exists in the function data, but the prose prompt doesn't ask "what are the key constants and their values?"
