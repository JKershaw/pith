...
2. For each task:
   - **Query Mode (NEW)**: Use `POST /query` with the task question
     - Records: answer, filesUsed, reasoning, candidatesConsidered
   - **Context Mode (optional)**: Use `GET /context?files=...` for comparison
   - Run Control agent (2-min time limit, grep/glob/read tools)
   - Judge both approaches using the appropriate scoring rubric
3. Compare results with previous benchmarks (2025-12-31-self-test-v4.md, 2025-12-30-self-test-v7.md)

### Query Mode Evaluation

With Phase 7 complete, the primary evaluation mode is now Query Mode:

```bash
curl -X POST http://localhost:3000/query \
  -H "Content-Type: application/json" \
  -d '{"query": "What are the main components of this codebase?"}'
```

This measures:

- File selection quality (did the planner choose the right files?)
- Answer quality (correctness, completeness, specificity, conciseness)
- End-to-end performance (time, token usage)
